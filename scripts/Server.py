...
_rpm_calls: list = []     # timestamps of recent Gemini calls
...
def _rpm_check():
    """Prune old call timestamps and return seconds to wait (0 = ok to proceed)."""
    now = _time.monotonic()
...
async def _gemini_generate(model_obj, content):
    """Throttle Gemini calls. Returns (response, None) or raises on hard errors."""
    loop = asyncio.get_running_loop()
    async with _get_lock():
        wait = _rpm_check()
        if wait > 0:
            raise RateLimitError(f"{int(wait) + 1}")
    return await loop.run_in_executor(None, lambda: model_obj.generate_content(content))

class RateLimitError(Exception):
    """Raised when we know we'd hit the RPM limit before even calling Gemini."""
    pass
...
async def chat(msg: Message):
    try:
        ...
        secs = str(rle)
        print(f"RPM limit reached — telling user to wait {secs}s")
        return {"reply": f"I need a quick breather — I've hit my message limit. Try again in {secs} seconds!"}
