...
_rpm_calls: list  = []     # timestamps of recent Gemini calls (global)
_RPM_LIMIT        = 13     # stay safely under the 15 RPM free-tier cap

# Per-user rate limit: max N messages per user per minute
_USER_RPM_LIMIT   = 4      # each user gets up to 4 messages/min
_user_rpm_calls: dict = collections.defaultdict(list)  # user_id/email → [timestamps]

# Request queue: serialise Gemini calls so bursts are queued, not rejected
_request_queue    = None   # asyncio.Queue, lazy-init
_QUEUE_MAXSIZE    = 30     # max waiting requests before hard rejection
...
def _rpm_check_global():
    """Prune old timestamps and return seconds to wait (0 = ok to proceed)."""
    now = _time.monotonic()
...
def _rpm_check_user(user_key: str):
    """Per-user rate check. Returns seconds to wait (0 = ok)."""
    now = _time.monotonic()
...
async def _gemini_generate(model_obj, content, user_key: str = "anon"):
    """
    Throttle Gemini calls with two layers:
      1. Per-user limit (_USER_RPM_LIMIT msgs/min) — fast-fail noisy users
      2. Global RPM limit — queue and wait rather than hard-reject
    """
    loop = asyncio.get_running_loop()

    # Layer 1: per-user check (no waiting — tell user to slow down)
    async with _get_lock():
        user_wait = _rpm_check_user(user_key)
    if user_wait > 0:
        raise RateLimitError(f"{int(user_wait) + 1}", per_user=True)

    # Layer 2: global RPM — queue the call; worker waits for a free slot
    fut = loop.create_future()
    try:
        _get_queue().put_nowait((model_obj, content, fut))
    except asyncio.QueueFull:
        raise RateLimitError("60", per_user=False)  # truly overloaded

    return await fut   # resolved by _rate_limited_worker

async def _rate_limited_worker():
    """Background worker that drains the queue respecting the global RPM cap."""
    loop = asyncio.get_running_loop()
    while True:
        model_obj, content, fut = await _get_queue().get()
        try:
            # Spin-wait until a global slot is free (max ~60 s)
            while True:
                async with _get_lock():
            ...
            result = await loop.run_in_executor(None, lambda: model_obj.generate_content(content))
            if not fut.done():
...
def load_all_persistent():
    ...
    print(f"[Astral] Loaded: {len(_user_stats)} users | {len(_reaction_log)} reactions | {len(_comments)} comment threads | {len(_allowed_emails)} allowed emails | {len(_user_memories)} memory users")
...
async def chat(msg: Message):
    try:
        ...
        if has_image:
            ...
            response = await _gemini_generate(model_obj, prompt_parts, user_key=user_key)
        else:
        ...
        err_str  = str(e).lower()
        # ── Gemini ResourceExhausted (slipped past our tracker) ──
        if "resourceexhausted" in err_name.lower() or "resource_exhausted" in err_str or "429" in err_str:
            print(f"Unexpected ResourceExhausted on {chosen_model}")
            return {"reply": "I'm at my message limit for this minute. Please wait 60 seconds and try again!"}
        # ── All other errors ──
        print(f"Critical error in /chat: {e}")
        import traceback; traceback.print_exc()
        return {"reply": f"Error: {err_name} — {str(e)[:120]}"}
...
async def rate_status(admin_email: str = ""):
    """Admin view of current RPM usage and queue depth."""
    if admin_email.lower() != ADMIN_EMAIL.lower():
        raise HTTPException(status_code=403, detail="Admin only.")
    now = _time.monotonic()
    # Count calls in last 60 s
