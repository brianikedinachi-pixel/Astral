...
async def chat(msg: Message):
    ...
    try:
        ...
        # Give enough tokens for full responses — 512 was too low and caused mid-sentence cutoffs
        # which caused the model to dump its formatting instructions into the output
        reply_max    = 2500 if (has_image or web_used) else 2000
        ...
        if has_image:
            ...
            # Build a clean user message — do NOT append "Astral:" suffix.
            # system_instruction already handles the persona; appending a
            # completion-style suffix fights against it and causes prompt leaking.
            ...
            # Clean user message: context blocks first, then the user's message.
            # No "Astral:" suffix — that completion trick leaks the system prompt
            # when the model hits the token limit mid-response.
        ...
        if "resourceexhausted" in err_name.lower() or "resource_exhausted" in err_str or "429" in err_str:
            ...
            try:
                fallback_obj = genai.GenerativeModel(
                    ...
                    system_instruction=get_full_system_prompt(),
                    generation_config=genai.GenerationConfig(
                        temperature=TEMPERATURE, top_p=TOP_P, max_output_tokens=2000,
                    ),
                )
                user_key     = (msg.user_email or msg.user_id or "anon").lower()
                web_instr    = "\n[Web context provided above. Use it to give accurate, cited answers.]\n" if web_used else ""
