...
async def chat(msg: Message):
    ...
    try:
        ...
        if has_image:
            import base64
            # Use inline_data dict format — works reliably across all SDK versions
            image_part = {
                "inline_data": {
                    "mime_type": msg.image_mime or "image/jpeg",
                    "data": msg.image_base64  # pass raw base64 string, not decoded bytes
                }
            }
            ...
            if gemini_history:
                # Use proper chat session for text — gives full conversation context
                try:
                    ...
                    # Normalise response so response.text always works
                    if not hasattr(response, 'text'):
                        response = type('R', (), {'text': response.candidates[0].content.parts[0].text})()
                except Exception:
                    # Fall back to standard generate if chat session fails
