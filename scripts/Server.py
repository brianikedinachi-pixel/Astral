...
def _safe_response_text(response) -> str:
    ...
    response.text raises AttributeError when:
      - candidates list is empty (model returned nothing)
      - finish_reason is SAFETY / RECITATION / OTHER (content blocked)
      - parts list is empty on the first candidate

    We handle all of these gracefully instead of letting the exception
    bubble up as a generic AttributeError to the user.
    """
    try:
    ...
    # Slow path: inspect candidates manually
    try:
        ...
        if finish_reason and finish_reason != 1:  # 1 = STOP (normal)
            ...
            reason = reason_map.get(int(finish_reason), str(finish_reason))
            if reason == "safety":
...
# Pinging every 5 minutes keeps the server awake 24/7.
...
async def chat(msg: Message):
    ...
    try:
        ...
        if has_image:
            ...
            # The old google-generativeai SDK does NOT have Part.from_bytes().
            # Pass a PIL Image directly — the SDK converts it internally.
            ...
            if gemini_history:
                ...
                response = await asyncio.get_event_loop().run_in_executor(
                    None, lambda: chat.send_message([pil_image, text_part])
                )
            else:
