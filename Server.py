...
async def _gemini_generate(model_obj, content, max_retries=3):
    """Throttle Gemini calls to stay under 15 RPM, with retry on ResourceExhausted."""
    global _last_gemini_call
    last_err = None
    for attempt in range(max_retries):
        async with _get_lock():
            wait = _MIN_INTERVAL - (_time.monotonic() - _last_gemini_call)
            if wait > 0:
        ...
        try:
            ...
            if is_rate_limit and attempt < max_retries - 1:
                wait_secs = 12 * (attempt + 1)   # 12s, 24s, 36s
                print(f"ResourceExhausted (attempt {attempt+1}), waiting {wait_secs}s…")
